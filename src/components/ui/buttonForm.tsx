...
export const ButtonForm: React.FC<ButtonFormProps> = ({
    ...
    return (
        <button
            className={`${!isSent ? 'bg-gray-300' : 'bg-blue-500'} text-gray-900 w-full md:w-1/2 rounded-full mx-auto group relative overflow-hidden transition-all duration-300 h-14 font-normal ${isSending || isSent ? 'md:w-14 aspect-square' : 'w-full'}`}
            onClick={handleSubmit}
            disabled={isSending}
        >
            <span className={`text-2xl flex items-center justify-between transition-all duration-300 pl-5 pr-3 ${isSending || isSent ? 'opacity-0 scale-0' : 'opacity-100 scale-100'}`}>
                Enviar
                <span className="bg-gray-950 rounded-full p-3 w-12 h-12 text-gray-300">
